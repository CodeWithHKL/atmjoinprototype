        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {recruitments.map((item) => (
              <div 
                key={item.id} 
                className={`group relative overflow-hidden rounded-[2.5rem] border backdrop-blur-xl p-8 flex flex-col transition-all duration-500 hover:scale-[1.02] ${item.color}`}
              >
                ...
                <div className="mb-8">
                  ...
                  <p className="text-zinc-400 text-sm font-semibold uppercase tracking-wider text-[11px]">{item.type} Track</p>
                </div>
                ...
                {item.status === "Apply Now" ? (
                  <Link href="/loggedin/recruitment/apply" className="block w-full relative z-10">
                    <button 
                      className={`mt-8 w-full py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg ${item.buttonColor}`}
                    >
                  ...
                  <button 
                    disabled
                    className={`mt-8 w-full py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-2 transition-all relative z-10 ${item.buttonColor}`}
                  >
                    {item.status === "Applied" ? (
          ...
          <div className="mt-12 p-8 rounded-[2.5rem] bg-black/40 backdrop-blur-2xl border border-white/10 flex flex-col md:flex-row items-center gap-8 shadow-2xl">
             ...
             <div>
               ...
               <p className="text-[11px] font-medium text-zinc-500 leading-relaxed max-w-3xl">
                 The <span className="text-emerald-500 font-bold">1/2</span> counter indicates your current commitment. If you have already applied to two branches, you must withdraw from one to open a new slot in accordance with National Defense recruitment policy.
               </p>
             </div>
          </div>
        </div>
