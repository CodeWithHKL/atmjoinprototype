...
export default function HelpPage() {
  ...
  return (
    <div className="relative min-h-screen bg-zinc-950 font-sans text-zinc-100 selection:bg-blue-500/30 overflow-x-hidden flex flex-col">
      
      {/* GLOBAL FIXED BACKGROUND (Camo Overlay) */}
      <div 
        className="fixed inset-0 z-0 bg-[url('/Camo.jpg')] bg-cover bg-center bg-fixed opacity-20 grayscale" 
        aria-hidden="true"
      />

      {/* Dark Overlay & Accents */}
      <div className="fixed inset-0 bg-black/60 z-0" />
      <div className="fixed top-[-10%] left-[-10%] h-[50%] w-[50%] rounded-full bg-blue-600/10 blur-[120px] z-0" />

      {/* MAIN CONTENT WRAPPER */}
      <div className="relative z-10 flex flex-col items-center w-full">
        <main className="w-full max-w-4xl mt-20 px-6 flex flex-col gap-8 pb-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {supportCategories.map((cat, i) => (
              <div 
                key={i}
                className="group relative flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl transition-all hover:bg-white/10 hover:border-white/20 shadow-2xl"
              >
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/5 border border-white/10 group-hover:scale-105 transition-transform">
                  {cat.icon}
                </div>
                <div>
          ...
          {/* FAQ Section */}
          <section className="flex flex-col gap-4 rounded-[2.5rem] border border-white/10 bg-black/40 p-8 backdrop-blur-2xl shadow-inner">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
              {faqs.map((faq, i) => (
                <button 
                  key={i}
                  className="flex items-center justify-between w-full rounded-2xl border border-white/5 bg-white/5 px-5 py-4 text-left text-xs font-medium text-zinc-300 transition-all hover:bg-white/10 hover:translate-x-1 group"
                >
                  {faq}
