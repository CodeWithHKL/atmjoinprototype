        ...
        <div className="flex-grow flex flex-col">
          {!isFinished ? (
            <>
              ...
              <div className="fixed top-24 md:top-28 w-full px-6 z-40 pointer-events-none">
                <div className="mx-auto max-w-2xl flex items-center gap-4 bg-black/60 backdrop-blur-2xl p-4 rounded-3xl border border-white/10 shadow-2xl pointer-events-auto">
                    ...
                    {questions.map((_, idx) => (
                      <button 
                        ...
                        className={`h-1.5 flex-1 rounded-full transition-all duration-300 hover:opacity-80 ${
                          ...
                          answers[idx] !== undefined ? "bg-zinc-600" : "bg-white/5"
                        }`} 
                      />
                    ))}
                  </div>
                  <div className="flex flex-col items-end min-w-[40px]">
                    <span className="font-mono text-[10px] text-emerald-400 font-bold leading-none mb-0.5">{completionPercentage}%</span>
              ...
              <main className="flex-grow flex flex-col items-center px-6 pt-52 pb-20">
                <div className="relative z-10 w-full max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-700">
                  <div className="flex flex-col items-center text-center mb-12">
                    <div className="h-20 w-20 rounded-3xl bg-black/40 backdrop-blur-xl border border-white/10 flex items-center justify-center text-emerald-400 mb-8 shadow-2xl group transition-transform hover:scale-110">
                      <ActiveIcon size={40} />
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-[0.5em] text-emerald-500/80 mb-4 flex items-center gap-2">
                      <Zap size={12} className="animate-pulse" /> {activeQ.category} Division
                    </span>
                    <h2 className="text-3xl md:text-4xl font-black uppercase tracking-tight leading-[1.1] max-w-xl text-balance">
                      {activeQ.text}
                    </h2>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 mb-10">
                    {["Strongly Agree", "Agree", "Neutral", "Disagree", "Strongly Disagree"].map((label, idx) => (
                      <button
                        ...
                        className={`group flex items-center justify-between p-6 rounded-2xl border-2 transition-all duration-300 active:scale-[0.98] backdrop-blur-sm ${
                      ...
                      >
                        ...
                        <div className={`h-6 w-6 rounded-full border-2 flex items-center justify-center ${
                          ...
                          {answers[currentStep] === idx && <div className="h-2 w-2 rounded-full bg-white animate-in zoom-in duration-300" />}
                        </div>
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="mx-auto max-w-md w-full">
                    <div className="bg-black/60 backdrop-blur-2xl border border-white/10 p-2 rounded-2xl flex items-center gap-2 shadow-3xl">
                      <button 
                        disabled={currentStep === 0}
                        onClick={() => setCurrentStep(prev => prev - 1)}
                        className="h-14 w-14 rounded-xl flex items-center justify-center bg-white/5 hover:bg-white/10 disabled:opacity-20 transition-all text-white"
                      ...
                      {currentStep === questions.length - 1 ? (
                         <button 
                            ...
                            className="flex-1 h-14 bg-emerald-500 text-black font-black uppercase tracking-tighter rounded-xl disabled:opacity-50 hover:bg-emerald-400 transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] text-[11px] active:scale-95"
                         >
                            Analyze Results
                         </button>
                      ) : (
                        <button 
                          disabled={answers[currentStep] === undefined}
                          onClick={() => setCurrentStep(prev => prev + 1)}
                          className="flex-1 h-14 bg-white text-black font-black uppercase tracking-tighter rounded-xl hover:bg-emerald-400 transition-colors disabled:opacity-50 text-[11px] active:scale-95"
                        >
                          Continue
                        </button>
                      )}
            ...
            <main className="flex-grow flex flex-col items-center justify-center p-6 text-center animate-in zoom-in-95 duration-700 pt-32 pb-20">
              <div className="h-24 w-24 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-400 mb-8 shadow-[0_0_50px_rgba(16,185,129,0.3)] border border-emerald-500/30">
              ...
              <div className="w-full max-w-md bg-black/60 border border-white/10 rounded-[2.5rem] p-10 mb-10 backdrop-blur-2xl shadow-2xl">
                {(() => {
                  const results = calculateResults();
                  const topMatch = results[0];
                  return (
                    <>
                      <div className="text-emerald-500 font-black text-7xl mb-2 tracking-tighter drop-shadow-[0_0_15px_rgba(16,185,129,0.4)]">{topMatch.percentage}%</div>
                        ...
                        {results.slice(1).map(r => (
                          <div key={r.branch} className="space-y-2">
                            ...
                            <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden border border-white/5">
                                <div className="h-full bg-zinc-500 transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(255,255,255,0.1)]" style={{ width: `${r.percentage}%` }} />
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              <Link href="/signup" className="h-16 px-12 rounded-2xl bg-white text-black font-black uppercase tracking-widest hover:bg-emerald-400 transition-all flex items-center justify-center hover:scale-105 active:scale-95 shadow-2xl text-[11px]">
